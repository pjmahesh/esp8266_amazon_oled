...
unsigned long myChannelNumber[2] = {681458, 681459};
const char * myWriteAPIKey[2] = {"DCRT9HFBK6PBCTUV","MEZE3KURC7UB0NYT"};//Freezer Tag(with ePTFE) ,Freezer Tag(withot enclosure)
...
{  
      ...
      if ( a == 0x0d   && b == 0x0a && c == 0xcf) // 0d:0a:cf>
      {
        macTime = millis();
        uploadFlag = true;
...
void setup() 
{
  ...
  display.println(F(" Setup Jan 29 1:25pm"));
  display.display();
  delay(2000);
  display.clearDisplay();
  display.display();
